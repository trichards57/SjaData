...
public class VehicleController(IVehicleService vehicleService) : ControllerBase
{
    ...
    public async Task<ActionResult<NationalVehicleReport>> GetVehiclesAsync([FromHeader(Name = "If-None-Match")] string? etag)
    {
        ...
        return Ok(await vehicleService.GetNationalReportAsync());
    }
    ...
    [ProducesResponseType(StatusCodes.Status304NotModified)]
    [RevalidateCache]
    public async Task<ActionResult<IAsyncEnumerable<VehicleSettings>>> GetVehiclesSettingsAsync([FromHeader(Name = "If-None-Match")] string? etag)
    {
        ...
        return Ok(vehicleService.GetVehicleSettings());
    }
    ...
    {
        await vehicleService.PostVehicleSettingsAsync(settings);
