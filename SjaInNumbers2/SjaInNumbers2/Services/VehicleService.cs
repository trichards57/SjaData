...
public class VehicleService(IDbContextFactory<ApplicationDbContext> contextFactory) : IVehicleService
{
    ...
    /// <inheritdoc/>
    public async IAsyncEnumerable<VehicleTypeStatus> GetNationalStatus()
    {
        using var context = await contextFactory.CreateDbContextAsync();
    ...
    /// <inheritdoc/>
    public async IAsyncEnumerable<VehicleSettings> GetVehicleSettings()
    {
        using var context = await contextFactory.CreateDbContextAsync();
    ...
    /// <inheritdoc/>
    public async Task<VehicleSettings?> GetVehicleSettingsAsync(int id)
    {
        using var context = await contextFactory.CreateDbContextAsync();
    ...
    /// <inheritdoc/>
    public async Task PostVehicleSettingsAsync(UpdateVehicleSettings settings)
    {
        using var context = await contextFactory.CreateDbContextAsync();
    ...
    private async IAsyncEnumerable<VorStatus> GetVorStatusesPrivateAsync(Region region)
    {
        ...
        await foreach (var v in context.Vehicles
            ...
            .GetActive()
            .GetForPlace(new Place() { Region = region })
            .AsAsyncEnumerable())
        {
            var now = DateTime.Today;
            var thirteenMonthsAgo = now.AddMonths(-13);
